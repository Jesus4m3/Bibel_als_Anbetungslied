...
### Seite fuer Veroeffentlichungen

Die Datei `veroeffentlichungen.html` ist die letzte Seite des gesamten Projekts und der letzte Eintrag der globalen Navigation.
Sie dokumentiert alle aktuell veroeffentlichten Liedseiten, die ueber die normale Navigation erreichbar sind.

- Wenn ein Lied neu veroeffentlicht und in die Navigation aufgenommen wird, soll auch `veroeffentlichungen.html` aktualisiert werden
...
- Die Tabelle auf `veroeffentlichungen.html` soll die Spalten `Datum`, `Bereich` und `Lied` verwenden
